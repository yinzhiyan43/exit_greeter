...
try:
    redis = redis.Redis.from_url('redis://:SBGMhVC7R92SrFHdp44UDF3zhR3t932YBmrVBEX8gps=@dev-indexes-cache.redis.cache.windows.net:6379/0')
    #redis.delete("vlcreader_key")
    d = redis.lrange("vlcreader_key",0 ,-1)
    for t in d:
        print(t.decode())
except:
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
